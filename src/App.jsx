...
const btnPrimary = {
  ...
  background: AMB, color: 'white', cursor: 'pointer',
...
function Badge({ status }) {
  ...
  return (
    <span style={{ fontSize: '11px', padding: '3px 8px', borderRadius: 4, background: s.bg, color: s.text, fontWeight: 500, whiteSpace: 'nowrap' }}>
      {s.label}
    </span>
  )
}
...
// ── Setup screen (shown when VITE_SCRIPT_URL is missing) ──────────────────────
    ...
    <div style={{ maxWidth: 560, margin: '4rem auto', padding: '0 1.5rem' }}>
      <div style={{ background: 'var(--bg-primary)', border: '0.5px solid var(--border)', borderRadius: 16, padding: '2rem' }}>
        <div style={{ width: 48, height: 48, borderRadius: 10, background: AMB, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1.25rem' }}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="white"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" fill="none"/></svg>
        </div>
        <h2 style={{ fontSize: '18px', fontWeight: 500, marginBottom: 8 }}>One more step</h2>
        <p style={{ fontSize: '14px', color: 'var(--text-secondary)', marginBottom: '1.5rem', lineHeight: 1.7 }}>
          Add your Google Apps Script URL to connect the app to your Google Sheet database.
        </p>
        <div style={{ background: 'var(--bg-secondary)', borderRadius: 10, padding: '1rem 1.25rem', marginBottom: '1.5rem' }}>
          ...
          {[
            'Deploy the apps-script/Code.gs to your Google Sheet (see README)',
            'Copy the deployment URL from Apps Script',
            'In Vercel → Settings → Environment Variables, add VITE_SCRIPT_URL = your URL',
        ...
        <p style={{ fontSize: '12px', color: 'var(--text-tertiary)' }}>Check the README.md in the project for detailed instructions.</p>
      </div>
    </div>
  )
}
...
function PinGate({ onUnlock }) {
  ...
  async function submit(pin) {
    setBusy(true)
    try {
      const res = await apiGet({ action: 'checkPin', pin })
      if (res.valid) { onUnlock(); return }
    } catch {}
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginBottom: '1rem', animation: shake ? 'tbox-shake 0.5s ease' : 'none', opacity: busy ? 0.6 : 1 }}>
          {digits.map((d, i) => (
            <input
              ...
              maxLength={1} value={d} disabled={busy}
...
function CustomerForm({ menu, onSubmit }) {
  ...
  // Returning-customer name search
  async function handleNameChange(val) {
    upd('name', val); clr('name')
    if (val.length >= 3) {
      try {
        const data = await apiGet({ action: 'searchNames', q: val })
  ...
  // Auto-fill phone + address for returning customer
  async function selectCustomer(name) {
    upd('name', name); setShowSuggestions(false)
    try {
  ...
  function reset() {
    setForm({ name: '', phone: '', address: '', slot: 'slot1', date: todayStr(), items: {}, notes: '' })
  ...
  if (submitted) return (
    <div style={{ maxWidth: 480, margin: '5rem auto', padding: '0 1rem' }}>
      <div style={{ background: 'var(--bg-primary)', border: '0.5px solid var(--border)', borderRadius: 16, padding: '2.5rem 2rem', textAlign: 'center' }}>
        ...
        <button onClick={reset} style={btnSecondary}>Place another order</button>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ maxWidth: 520, margin: '0 auto', padding: '1.5rem 1rem 4rem' }}>
      ...
      <Sec title="Your details">
        <Fld label="Full name" error={errors.name}>
          <div style={{ position: 'relative' }}>
            <input style={inpStyle} placeholder="e.g. Priya Sharma" value={form.name}
        ...
        <Fld label="Mobile number" error={errors.phone}>
          <input style={inpStyle} placeholder="10-digit number" value={form.phone} maxLength={10}
            onChange={e => { upd('phone', e.target.value.replace(/\D/g, '')); clr('phone') }} />
        </Fld>

        <Fld label="Delivery address" error={errors.address}>
          <div style={{ position: 'relative' }}>
            <textarea style={{ ...inpStyle, resize: 'none', paddingRight: 38 }}
      ...
      <Sec title="Special instructions" optional>
        <textarea style={{ ...inpStyle, resize: 'none' }} placeholder="Allergies, spice level, any requests…"
          rows={2} value={form.notes} onChange={e => upd('notes', e.target.value)} />
      </Sec>

      <button onClick={handleSubmit} disabled={busy}
        style={{ ...btnPrimary, width: '100%', padding: '12px', fontSize: '15px', opacity: busy ? 0.7 : 1, cursor: busy ? 'not-allowed' : 'pointer' }}>
...
function AdminView({ orders, menu, setOrders, setMenu, onLock }) {
  ...
  const [saving,       setSaving]       = useState(null) // tracks which row is saving
  ...
  async function saveEdit(updated) {
    ...
    setEditOrder(null)
    try { await apiPost('updateOrder', { order: updated }) } catch {}
  }
  ...
  function exportExcel() {
    ...
    try {
      const rows = filtered.map(o => ({
        'Date':       o.date,
        'Slot':       SLOT_LABELS[o.slot],
        'Name':       o.name,
        'Phone':      `'${o.phone}`,
        'Address':    o.address,
        'Items':      Object.entries(o.items || {}).map(([k, v]) => `${k} x${v}`).join(', '),
        'Qty Total':  Object.values(o.items || {}).reduce((a, b) => a + b, 0),
        'Notes':      o.notes || '',
        'Status':     STATUS[o.status]?.label || o.status,
        'Payment':    o.payment === 'paid' ? 'Paid' : 'Pending',
        'Ordered At': fmtTime(o.createdAt),
      }))
      const ws = XLSX.utils.json_to_sheet(rows)
      ws['!cols'] = [10, 16, 16, 13, 30, 36, 8, 20, 14, 10, 12].map(w => ({ wch: w }))
      const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, 'Orders')
      XLSX.writeFile(wb, `tiffinbox-${filterDate || 'all'}.xlsx`)
    } catch { alert('Export failed') }
    setExporting(false)
  }

  const selStyle = { padding: '7px 10px', borderRadius: 8, border: '0.5px solid var(--border)', background: 'var(--bg-primary)', color: 'var(--text-primary)', fontSize: '13px', cursor: 'pointer', outline: 'none', fontFamily: 'inherit' }
    ...
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '1.5rem 1rem 4rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', borderBottom: '0.5px solid var(--border)', marginBottom: '1.25rem' }}>
        ...
        <button onClick={onLock}
          style={{ marginLeft: 'auto', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-tertiary)', padding: '4px 8px', display: 'flex', alignItems: 'center', gap: 5, fontSize: '12px', fontFamily: 'inherit' }}>
      ...
      {tab === 'orders' ? (
        <>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', marginBottom: '1rem' }}>
            ...
            <select value={filterSlot} onChange={e => setFilterSlot(e.target.value)} style={selStyle}>
            ...
            <input placeholder="Search name, phone, address…" value={search} onChange={e => setSearch(e.target.value)} style={{ ...selStyle, minWidth: 200 }} />
            <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <button onClick={exportExcel} disabled={exporting} style={{ ...btnSecondary, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {/* Table */}
          {filtered.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '4rem 2rem', color: 'var(--text-secondary)', background: 'var(--bg-primary)', borderRadius: 12, border: '0.5px solid var(--border)' }}>
                    ...
                    {filtered.map((o, i) => (
                      <tr key={o.id} style={{ borderBottom: i < filtered.length - 1 ? '0.5px solid var(--border)' : 'none', background: selected.has(o.id) ? AMB_BG : 'transparent', opacity: saving === o.id ? 0.6 : 1, transition: 'opacity 0.15s' }}>
                        <td style={{ padding: '10px 12px' }}>
                          <input type="checkbox" checked={selected.has(o.id)} onChange={() => setSelected(s => { const n = new Set(s); n.has(o.id) ? n.delete(o.id) : n.add(o.id); return n })} style={{ cursor: 'pointer' }} />
                        </td>
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
      <div style={{ background: 'var(--bg-primary)', borderRadius: 16, border: '0.5px solid var(--border)', width: '100%', maxWidth: 540, maxHeight: '90vh', overflow: 'auto', padding: '1.5rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
          <Fld label="Name"><input style={inpStyle} value={form.name} onChange={e => upd('name', e.target.value)} /></Fld>
          <Fld label="Phone"><input style={inpStyle} value={form.phone} onChange={e => upd('phone', e.target.value)} /></Fld>
        </div>
        <div style={{ marginBottom: 10 }}>
          <Fld label="Address"><textarea style={{ ...inpStyle, resize: 'none' }} rows={2} value={form.address} onChange={e => upd('address', e.target.value)} /></Fld>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 10 }}>
          <Fld label="Date"><input style={inpStyle} type="date" value={form.date} onChange={e => upd('date', e.target.value)} /></Fld>
          <Fld label="Slot">
            <select style={{ ...inpStyle, cursor: 'pointer' }} value={form.slot} onChange={e => upd('slot', e.target.value)}>
              <option value="slot1">Slot 1 (Morning)</option>
              <option value="slot2">Slot 2 (Afternoon)</option>
            </select>
          </Fld>
          <Fld label="Status">
            <select style={{ ...inpStyle, cursor: 'pointer' }} value={form.status} onChange={e => upd('status', e.target.value)}>
              {Object.entries(STATUS).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
            </select>
          </Fld>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: '1.25rem' }}>
          <Fld label="Notes"><input style={inpStyle} value={form.notes || ''} onChange={e => upd('notes', e.target.value)} placeholder="Special instructions" /></Fld>
          <Fld label="Payment">
            <select style={{ ...inpStyle, cursor: 'pointer' }} value={form.payment || 'pending'} onChange={e => upd('payment', e.target.value)}>
              <option value="pending">Pending</option>
              <option value="paid">Paid</option>
            </select>
          </Fld>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          ...
          <button onClick={() => onSave(form)} style={btnPrimary}>Save changes</button>
        </div>
      </div>
    </div>
  )
...
function SettingsTab({ menu, setMenu }) {
  ...
  async function saveMenu(newMenu) {
    setMenu(newMenu)
    setSavingMenu(true)
    try { await apiPost('updateMenu', { menu: newMenu }) } catch {}
    setSavingMenu(false)
  }

  async function addItem() {
    const t = newItem.trim(); if (!t || menu.includes(t)) return
    await saveMenu([...menu, t]); setNewItem('')
  }
  async function removeItem(item) { await saveMenu(menu.filter(m => m !== item)) }

  function handleDrop(idx) {
    if (dragIdx === null || dragIdx === idx) return
    const next = [...menu]; const [moved] = next.splice(dragIdx, 1); next.splice(idx, 0, moved)
    saveMenu(next); setDragIdx(null); setOverIdx(null)
  }
  ...
  async function changePin() {
    ...
    try {
      ...
      setPinCur(''); setPinNew1(''); setPinNew2('')
      setPinMsg({ err: false, text: 'PIN updated successfully' })
    } catch {
      setPinMsg({ err: true, text: 'Could not update PIN. Check your connection.' })
    }
  }

  const pinInp = { ...inpStyle, letterSpacing: '0.25em' }
    ...
    <div style={{ maxWidth: 480, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      <div style={{ background: 'var(--bg-primary)', borderRadius: 12, border: '0.5px solid var(--border)', padding: '1.25rem' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input placeholder="Add new dish…" value={newItem} onChange={e => setNewItem(e.target.value)} onKeyDown={e => e.key === 'Enter' && addItem()} style={{ ...inpStyle, flex: 1 }} />
      ...
      <div style={{ background: 'var(--bg-primary)', borderRadius: 12, border: '0.5px solid var(--border)', padding: '1.25rem' }}>
        ...
        <button onClick={changePin} style={btnPrimary}>Update PIN</button>
      </div>
    </div>
  )
}
...
export default function App() {
  ...
  async function loadData() {
    ...
    try {
      const [ordersData, menuData] = await Promise.all([
        apiGet({ action: 'getOrders' }),
        apiGet({ action: 'getMenu'   }),
      ])
      setOrders(Array.isArray(ordersData) ? ordersData : [])
      setMenu(Array.isArray(menuData) && menuData.length ? menuData : DEFAULT_MENU)
    } catch (e) {
      setError('Could not reach the server. Check your Apps Script URL and make sure it is deployed and accessible.')
    }
    setLoading(false)
  }
  ...
  // Missing env var → show setup screen
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 52 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
          ...
          {view === 'admin' && adminUnlocked && (
            <span style={{ fontSize: '11px', padding: '2px 8px', borderRadius: 4, background: AMB_BG, color: AMB_DARK, fontWeight: 500 }}>Admin</span>
          )}
        </div>
